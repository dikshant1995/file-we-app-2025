// Comparison of loan eligibility for government employee with ₹75,000 salary
...
console.log('Profile: Government Employee with ₹75,000 monthly salary');
...
console.log('\nBandhan Bank Calculation (FOIR Only):');
console.log('===================================');

const bandhanResult = calculateBandhanEligibility(applicantData);

if (bandhanResult.eligible) {
  console.log(`Eligible: ${bandhanResult.eligible}`);
  console.log(`Bank: ${bandhanResult.bankName}`);
  console.log(`Company Category: ${bandhanResult.companyCategory}`);
  console.log(`Calculation Method: ${bandhanResult.calculationMethod}`);
  console.log(`FOIR Percentage: ${(bandhanResult.foirPercentage * 100)}%`);
  console.log(`FOIR Cap: ₹${bandhanResult.details.foirCap.toLocaleString()}`);
  console.log(`Available EMI: ₹${bandhanResult.details.availableEMI.toLocaleString()}`);
  console.log(`FOIR-Based Loan Amount: ₹${bandhanResult.details.foirLoanAmount.toLocaleString()}`);
  console.log(`Maximum Loan Amount: ₹${bandhanResult.loanAmount.toLocaleString()}`);
  console.log(`Monthly EMI: ₹${bandhanResult.monthlyEMI.toLocaleString()}`);
  console.log(`Interest Rate: ${bandhanResult.interestRate}%`);
} else {
  console.log(`Eligible: ${bandhanResult.eligible}`);
  console.log(`Reason: ${bandhanResult.reason}`);
}

console.log('\n=== Summary Comparison ===');
const results = [
  { bank: 'HDFC', amount: hdfcResult.eligible ? hdfcResult.loanAmount : 0 },
  { bank: 'Bandhan', amount: bandhanResult.eligible ? bandhanResult.loanAmount : 0 },
...
console.log('\n=== Bandhan Bank Specific Notes ===');
console.log('✓ Bandhan Bank uses 70% FOIR for salaries ≥ ₹75,000 as specified');
console.log('✓ Bandhan Bank uses FOIR calculation only');
console.log('✓ For ₹75,000 salary, Bandhan offers ₹27,58,211 loan amount');