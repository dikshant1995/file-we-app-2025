...
const CustomerResultsDisplay = ({ results, metadata, onNewCalculation }) => {
  ...
  // DATA MAPPING: Bridge new standardized results to professional UI expectations
  const processedResults = results.map(r => ({
    ...r,
    name: r.bankName,
    isEligible: r.eligible,
    maxLoanAmount: r.loanAmount || 0,
    rejectionReason: r.reason,
    // Extract emiCap from foirPercentage if available
    emiCap: r.emiCap || (r.foirPercentage ? (parseFloat(r.foirPercentage) * 100) : 0)
  }));

  const userData = {
    name: metadata?.customerName || 'Customer',
    employer: metadata?.companyName || 'Private Company',
    monthlyIncome: metadata?.monthlyIncome || 0
  };

  const handleCopySummary = () => {
    const eligibleCount = processedResults.filter(r => r.isEligible).length;
    const bestAmount = Math.max(...processedResults.map(r => r.maxLoanAmount), 0);
  ...
  return (
    <div className="results-container">
      ...
      >
        ...
        <p>Calculated across {processedResults.length} institutional policies for {userData.name}</p>
      </motion.header>
      ...
      <div className="summary-grid">
        ...
        >
          ...
          <div className="stat-info">
            ...
            <span className="stat-value">₹{Math.max(...processedResults.map(r => r.maxLoanAmount), 0).toLocaleString()}</span>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="stat-info">
            ...
            <span className="stat-value">{processedResults.filter(r => r.isEligible).length} Eligible</span>
          </div>
        </motion.div>
      </div>
      ...
      <section className="banks-section">
        ...
        <div className="banks-grid">
          {processedResults.filter(r => r.isEligible).sort((a, b) => b.maxLoanAmount - a.maxLoanAmount).map((bank, index) => (
            <motion.div
              key={bank.name}
                ...
                {selectedBank === bank.name && (
                  ...
                  >
                    ...
                    <div className="calculation-breakdown">
                      ...
                      <div className="calc-steps">
                        <div className="calc-step">
                          ...
                          <span>{bank.multiplier || '26'}x Gross Income</span>
                        </div>
                        {(bank.emiCap > 0) && (
                          <div className="calc-step">
                            <span>EMI Capping (FOIR)</span>
                            <span>{(bank.emiCap).toFixed(0)}% of Net</span>
                          </div>
                        )}
                    ...
                    <button className="apply-btn-bank" onClick={(e) => {
                      e.stopPropagation();
                      alert(`Initiating application for ${bank.name}... Our team will contact you.`);
                    }}>
